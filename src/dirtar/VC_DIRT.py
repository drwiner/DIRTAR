...
# Discovery of Inference Rules from Text using Verb Context (VC-DIRT)
...
# another round of slots added v_i-1, v_i+1, the verb that occurs before and the verb that occurs subsequently
...
def readCorpus(clause_file):
	...
	with open(clause_file, 'r') as clauses:
		for i, line in enumerate(clauses):
			...
			if last_clause != path:
				VStream.append(Triple(last_clause, path, None))
				VStream.append(Triple(None, last_clause, path))
				last_clause = path
...
def loadDatabase(db, filtered_Tinstances):
	...
	for x, path, y in filtered_Tinstances:
		if path is None or path == 'none' or path == 'None':
			continue
...
def pathSim_vc(p1, p2, db, vc_db):
	svo_MI = pathSimdb(p1, p2, db)
	vc_MI = pathSimdb(p1, p2, vc_db)
	return vc_MI + svo_MI / 2


def pathSim_vc_logsvo(p1, p2, db, vc_db):
	svo_MI = pathSimdb(p1, p2, db)
	vc_MI = pathSimdb(p1, p2, vc_db)
	return vc_MI + (1/log2(1 + svo_MI))
...
if __name__ == '__main__':
	...
	streams = [TStream, FCTStream]
	# s_names = ['tstream', 'ctstream', 'ftstream', 'fctstream', 'wstream']
	s_names = ['vc_stream', 'vc_fctstream']
	...
	# used in conjunction with other dbs.
	verb_context_db = rec_dd()


	# databases = [triple_database, triple_dep_filtered, triple_collapsed, triple_dep_filtered_collapsed, triple_W_db]
	databases = [triple_database, triple_dep_filtered_collapsed]
	...
	# For Loading verb contexts into database
	dp, dmf, pi, pimf, vc_instances = apply_MinfreqFilter(VStream, 'vc', min_freq)
	print('loading verb context database')
	loadDatabase(verb_context_db, vc_instances)
	print('updating MI')
	verb_slot_count = dict()
	verb_count = dict()
	updateMI(verb_context_db, verb_slot_count, verb_count)
	print('after MI, dumping database')
	save_database(verb_context_db, 'vc')
	...
	# dp, dmf, pi, pimf, ftinstances = apply_MinfreqFilter(MStream, 'vc_mstream', min_freq)
	...
	# # CMStream
	# triple_multi_slot = rec_dd()
	# dp, dmf, pi, pimf, ftinstances = apply_MinfreqFilter(CMStream, 'vc_cmstream', min_freq)
	# loadDatabase_multislot(triple_multi_slot, ftinstances)
	# updateMI_multislot(triple_multi_slot, dict(), defaultdict(int))
